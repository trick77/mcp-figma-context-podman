...
- **Don't bind-mount system dirs** (e.g. `/etc/pki/...`, `/usr/local/share/ca-certificates/`). That mauls the host's SELinux labels and breaks `update-ca-trust`. Project-local or `$HOME`-scoped bind mounts are fine — current examples: `./certs/` → `/etc/ssl/ca-anchors` (ro) and `$IMAGES_HOST_DIR` at the same path inside the container (rw, same-path so the tool-returned path is host-valid). `IMAGES_HOST_DIR` is required in `.env` because compose can't expand `$HOME` inside its own defaults — don't try to add a `${IMAGES_HOST_DIR:-$HOME/...}` fallback, podman-compose treats the default as a literal named-volume name.
