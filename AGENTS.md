...
- **Don't bind-mount system dirs** (e.g. `/etc/pki/...`, `/usr/local/share/ca-certificates/`). That mauls the host's SELinux labels and breaks `update-ca-trust`. Project-local bind mounts are fine — current examples: `./certs/` → `/etc/ssl/ca-anchors` (ro) and `$IMAGES_HOST_DIR` (operator-set in `.env`, no default) at the same path inside the container (rw, same-path so the tool-returned path is host-valid).
- **Don't add a compose default for `IMAGES_HOST_DIR`.** Two failure modes: (1) `${IMAGES_HOST_DIR:-$HOME/...}` — podman-compose doesn't recurse into `:-default`, so `$HOME` ends up literal and the path is misclassified as a named volume (`volume [$HOME/...] not defined`). (2) A literal default like `/var/tmp/foo` makes downloads land in a surprise location operators didn't choose. Keep `IMAGES_HOST_DIR` required (`${IMAGES_HOST_DIR:?...}`) so missing-var fails fast with a clear message.
