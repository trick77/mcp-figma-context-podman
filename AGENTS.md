...
- **Don't bind-mount system dirs** (e.g. `/etc/pki/...`, `/usr/local/share/ca-certificates/`). That mauls the host's SELinux labels and breaks `update-ca-trust`. Project-local bind mounts are fine — current examples: `./certs/` → `/etc/ssl/ca-anchors` (ro) and `$IMAGES_HOST_DIR` (default `/var/tmp/figma-context-mcp`) at the same path inside the container (rw, same-path so the tool-returned path is host-valid).
- **Don't reintroduce a `${IMAGES_HOST_DIR:-$HOME/...}` fallback in compose.yaml.** podman-compose doesn't recurse into `:-default`, so `$HOME` is taken as a literal string and the resulting "path" is misclassified as a named volume — `up`/`pull` fail with `volume [$HOME/...] not defined in top level`. Keep the default a literal absolute path.
