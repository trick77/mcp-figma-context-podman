...
- **Don't bind-mount system dirs** (e.g. `/etc/pki/...`, `/usr/local/share/ca-certificates/`). That mauls the host's SELinux labels and breaks `update-ca-trust`. Project-local or `$HOME`-scoped bind mounts are fine — current examples: `./certs/` → `/etc/ssl/ca-anchors` (ro) and `$IMAGES_HOST_DIR` (default `$HOME/.cache/figma-context-mcp`) at the same path inside the container (rw, same-path so the tool-returned path is host-valid).
