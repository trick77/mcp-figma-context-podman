...
- **Don't add `--skip-image-downloads`.** The `./images/` bind mount exists for the `download_figma_images` tool — disabling it leaves a dead mount.
...
- **Don't bind-mount system dirs** (e.g. `/etc/pki/...`, `/usr/local/share/ca-certificates/`). That mauls the host's SELinux labels and breaks `update-ca-trust`. Project-local bind mounts under the repo root are fine — `./certs/` → `/etc/ssl/ca-anchors` (ro) and `./images/` → `/home/node/images` (rw) are the current examples.
